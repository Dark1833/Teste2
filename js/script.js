document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleNextRound() {
        ...
        // ENVIA OS DADOS PARA O BOT ANTES DE MUDAR O ESTADO
        postResultsToDiscord({
            roundNumber: state.roundNumber,
            winners: winners,
            losers: losers
        });

        state.currentRoundPlayers = shuffleArray(nextRoundPlayers); // Embaralha para a próxima rodada
    ...
    async function postResultsToDiscord(roundData) {
        try {
            const response = await fetch('/api/post_results', {
                method: 'POST',
                headers: {
            ...
            console.log('Resposta do bot:', result.success);

        } catch (error) {
            console.error('Falha na comunicação com o bot:', error);
        }
    }
